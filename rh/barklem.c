...
// HNS: Making RH to read the correct orbital quantum number as done by Rene
// The abo_level input is given in the atom model. 
// 

  if ((atom->abo_level[i] != -1) && (atom->abo_level[j] != -1)) {
      Ll =  atom->abo_level[i];
      Lu =  atom->abo_level[j];
      determined = TRUE;
  } else {
