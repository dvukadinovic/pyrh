/* ------- file: -------------------------- readmolecule.c ----------
       ...
       Last modified: Fri Jul 13 13:39:16 2018 --
...
void readMolecularLines(struct Molecule *molecule, char *line_data)
{
  ...
  if (strstr(format_string, "GOORVITCH94")) {
    ...
    if (atmos.Stokes) {
      /* -- Employ the fact that the CO rotation-vibration lines are
            between the X ^1\Sigma^+ states, automatically implying
            Hund's case B, \Lambda = 0, \Sigma = 0 --  -------------- */
      
      mrt->polarizable = TRUE;
      
      mrt->Hundi = CASE_B;
      mrt->Hundj = CASE_B;
      strncpy(mrt->parityi, "E\0", 2);
      strncpy(mrt->parityj, "E\0", 2);
      mrt->subi = 1;
      mrt->subj = 1;
      mrt->Lambdai = 0;
      mrt->Lambdaj = 0;
